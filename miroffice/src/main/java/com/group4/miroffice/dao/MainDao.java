...
public interface MainDao {
	...
	@Select("select sche_no, emp_no, sche_start_date, sche_end_date, sche_title, sche_allday, color from schedule where emp_no = #{empNo} limit 10 ")
	List<Schedule> getScheduleList(int empNo);
	
	@Select("select forum_no, emp_no, dept_no, forum_title, forum_text, forum_date, forum_count, forum_notice, forum_files, emp_name from forum natural join employee order by forum_notice desc, forum_date desc limit 10")
