...
public class FindIdResetPwService {
	...
	/*
	 * public void sendMail(String temppw, String name, String email) throws
	 * Exception {
	 * 
	 * Mail mail = new Mail(); mail.setTo(email);
	 * mail.setTitle("[미르 오피스] 임시 비밀번호 전송 이메일"); // 메일 제목
	 * mail.setMessage("안녕하세요. 미르 오피스 입니다. 사원님의 임시 비밀번호는" + temppw +
	 * "입니다. 로그인 후 비밀 번호 변경 해주세요."); // 메일 내용
	 * 
	 * SimpleMailMessage message = new SimpleMailMessage();
	 * message.setSubject(mail.getTitle()); message.setText(mail.getMessage());
	 * message.setTo(email); mailSender.send(message);
	 * 
