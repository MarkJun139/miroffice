      ...
      <div class="row row-cols-1">
														...
														<td><c:choose>
																<c:when test="${project.projectpercent == 100}">
																	<span class="badge badge-secondary"
																		style="background-color: var(--bs-success)">완료</span>
																</c:when>
																<c:when test="${project.projectdiffdate < 0}">
																	<span class="badge badge-secondary"
																		style="background-color: var(--bs-secondary)">기한
																		만료</span>
																</c:when>
																<c:when test="${project.projectpercent < 100}">
																	<span class="badge badge-secondary"
																		style="background-color: var(--bs-warning)">진행중</span>
																</c:when>
															</c:choose></td>
															...
															<div class="shadow-none progress bg-soft-primary w-100"
																...
																<c:if test="${project.projectpercent < 100}">
																	<div class="progress-bar bg-primary"
																		...
																		aria-valuenow="${project.projectpercent}"
									...
									<div class="row">
										...
										<div class="col-md-6" style="text-align: right">
											...
											<p class="col-12" id="nowClock" style="font-size: 15px">
												<span>현재시간</span> <span id="ampm"></span> <span id="hour"></span>시
												<span id="minute"></span>분 <span id="second"></span>초
											</p>
										</div>
									</div>
								</div>
													...
													<c:choose>
														<c:when test="${startTime.year == 70 }">
															...
															<div class="grid-cols d-grid gap-card">
																<a href="/main/start"
																	class="p-2 btn btn-info text-uppercase">출근 등록</a>
															</div>
														</c:when>
														...
														<c:when test="${startTime.year != 70 }">
															<p class="card-text">
																<fmt:formatDate pattern="HH시 mm분 ss초"
																	value="${startTime}" />
															</p>
															<div class="grid-cols d-grid gap-card">
																<button class="p-2 btn btn-info text-uppercase" disabled>출근
																	등록</button>
															</div>
														</c:when>
												...
												<div class="card-body text-center">
													...
													<c:choose>
														<c:when test="${endTime.year == 70 }">
															...
															<div class="grid-cols d-grid gap-card">
																<button class="p-2 btn btn-primary text-uppercase">퇴근
																	등록</button>
															</div>
														</c:when>
						...
						<script>
							window.onload = function() {
								...
								function getClock() {
									...
									minute.innerText = addZero(date
											.getMinutes());
									second.innerText = addZero(date
											.getSeconds());
								...
								<div class="card-body">
									...
									<c:forEach items="${scheduleList}" var="sche">
										<div class="card rounded-0 shadow-lg "
											style="border-left: 5px solid ${sche.color};">
											<div class="card-body">
		...
		<div class="btn-download">
			<a class="btn btn-success px-3 py-2"
				...
				target="_blank"> <svg class="icon-24" width="24"
