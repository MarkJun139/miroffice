...
<body class="  ">
	...
	<main class="main-content">
		...
		<div class="conatiner-fluid content-inner mt-n5 py-0">
			...
			<div class="overflow-hidden card" data-aos="fade-up"
				...
				<div class="card-body"
					style="border: 1px; float: left; margin-right: 20px;">
					...
					<div class="col-md-6" style="padding: 5px; border: 1px">
						<form name="form" id="form" method="post"
							...
							<div style="display: inline;">
								...
								<p
									style="position: relative; top: 10px; right: 88px; display: inline; text-align: left;">
									<fmt:formatDate value="${list.forumDate }"
							...
							<div
								style="margin-top: 2em; border-top-style: solid; border-top-color: lightgreen;">
									...
									<c:forEach items="${clist }" var="f">
										...
										<div id="comment"
											...
											<div id="editbt">
												...
												<c:choose>
													<c:when test="${userName == f.empNo}">
														<button type="button" id="cedit" class="btn btn-light"
															style="float: right;"
															onclick="return commentEdits(${f.commentNo})">수정</button>
													</c:when>
												</c:choose>
											</div>
											<div id="cancelbt" style="display: none;">
												...
												<c:choose>
													<c:when test="${userName == f.empNo}">
														<button type="button" id="cdelete" class="btn btn-light"
															style="float: right;"
															onclick="return commentCancel(${f.commentNo})">취소</button>
													</c:when>
												</c:choose>
											</div>
											<div id="perfectbt" style="display: none;">
												...
												<c:choose>
													<c:when test="${userName == f.empNo}">
														<button type="button" id="cedit" class="btn btn-light"
															style="float: right;"
															onclick="return commentPerfect(${f.commentNo})">완료</button>
													</c:when>
												</c:choose>
											</div>
											<div id="editdiv" style="display: none;">
												<input type="hidden" value=${f.commentNo }>
												<textarea style="width: 700px;" id="commentEdit"
													name="commentEdit">${f.commentText }</textarea>
											</div>
											<div id="readdiv">
												<input type="hidden" value=${f.commentNo }>
												<p
													style="padding-left: 60px; margin-top: .3em; color: black;">
													${f.commentText }</p>
											</div>
											<p
												style="padding-left: 60px; margin-top: .5em; display: inline;">
												<fmt:formatDate value="${f.commentDate }"
													pattern="yy.MM.dd HH:mm" />
											</p>
										</div>
									</c:forEach>
						...
						<form method="post" name="form2" id="form2">
							<div>
								<b style="padding-left: 5px; color: black;"><sec:authentication
										property="principal.users.empName" /> <sec:authentication
										property="principal.users.empRank" /></b> <input type="hidden"
	...
	function commentWrite() {
 	...
 	function commentPerfect(no) {
 		var ceditv
 		$('#editdiv input').each(function(){
			if($(this).val() == no){
				ceditv = $(this).siblings('textarea').val()
			}
 		})
		if(ceditv == ''){
			alert('댓글 내용을 입력하세요');
			...
			var submitObj = new Object();
			submitObj.commentNo = no;
			submitObj.commentText = ceditv;
			$.ajax({
				type : "POST",	
				url : "../comment/edit",
				contentType: "application/json; charset=utf-8",
					...
					if($(this).val() == no){
						...
						$('#editdiv input').each(function(){
							if($(this).val() == no){
								...
								$('#deletebt input').each(function(){
									if($(this).val() == no){
										$(this).parent().css("display", "");
										
									}
								})
								$('#editbt input').each(function(){
									if($(this).val() == no){
										$(this).parent().css("display", "");
